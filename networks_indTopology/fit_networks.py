...
def main(sub, confspec = '32Pscrub3BPfilter',
    ...
    fn_path = op.join(target_folder, f'sub-{sub}_module_mapping_infomap_hemi-R_thresh-{thresh_conn}_confspec-{confspec}.npy')
    if  op.exists(fn_path):
        print(f'Module mapping file already exists: {fn_path}. Skipping to plotting')
    else:
        for hemi in ['L', 'R']:    # fit for each hemisphere
            cm_hemi = cm_hemi_L if hemi == 'L' else cm_hemi_R

            conn_matrix = cm_hemi # Proportion of connections to keep
    ...
    for i_hemi, hemi in enumerate(['L','R']):
        modules = np.load(op.join(target_folder, f'sub-{sub}_module_mapping_infomap_hemi-{hemi}_thresh-{thresh_conn}_confspec-{confspec}.npy'))
        N = 9354 if hemi == 'L' else 9361

        all_nodes = np.arange(N)
        returned_nodes = np.array([node for node in modules[:, 0]])
        returned_modules = np.array([module_id for module_id in modules[:, 1]])
        ...
        for i,view in enumerate(views):
            ...
            nplt.plot_surf(surf_mesh=surf_mesh , surf_map= map, avg_method = 'median',# infl_right # pial_right
                    ...
                    bg_map=bg_map, bg_on_data=True,darkness=0.7, axes=axes[i_hemi,i]) 
...
if __name__ == '__main__':  
    ...
    parser.add_argument('--confspec', default='32Pscrub3BPfilter', type=str, help='Configuration specification')
