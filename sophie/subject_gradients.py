...
def main(sub,ses,bids_folder_cm, specification='', n_components=10):

    sub = '%02d' % int(sub)
    target_dir = op.join(bids_folder_cm, 'derivatives', 'gradients', f'sub-{sub}')
    if not op.exists(target_dir):
        os.makedirs(target_dir)

    # Define the stimulus types
    stimulus_types = ['stimulus_1', 'stimulus_2', 'remaining']
    ...
    for stim in stimulus_types:
        ...
        g_ref = np.load(op.join(bids_folder_cm,'derivatives', 'gradients','sub-All', f'stimulus_{stim}', f'sub-All_gradients_space-fsaverag5_n10_stimulus_{stim}.npy')) # 
        ...
        # save results
        np.save(op.join(target_dir,f'sub-{sub}_lambdas_space-fsaverag5_n10{specification}_stimulus_{stim}.npy'), gm.lambdas_) # save all together
        gm_= gm.gradients_.T 
        ...
        np.save(op.join(target_dir,f'sub-{sub}_gradients_space-fsaverag5_n10{specification}_stimulus_{stim}.npy'), grad) # save all together
        gm_ = gm.aligned_.T
        grad = [None] * n_components
        for i, g in enumerate(gm_): # gm.gradients_.T
            grad[i] = map_to_labels(g, labeling_noParcel, mask=mask, fill=np.nan)
        np.save(op.join(target_dir,f'sub-{sub}_g-aligned_space-fsaverag5_n10{specification}_stimulus_{stim}.npy'), grad) # save all together    
...
if __name__ == '__main__':
    ...
    main(cmd_args.subject, cmd_args.session, cmd_args.bids_folder_cm, 
