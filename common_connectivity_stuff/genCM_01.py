...
def main(sub, bids_folder, confspec='36P', ses=1, task='magjudge',
        scrubbing=True, scrub_thresh=0.3, 
        run_FD_filter=True, frames_per_run_thresh=104,
        bp_filtering=True, lower_bpf=0.01, upper_bpf=0.08):  
    ...
    clean_ts, N_valid_runs = cleanTS(sub,fmriprep_confounds_include=fmriprep_confounds_include, bids_folder=bids_folder, 
                task=task, ses=ses, 
    ...
    fn = op.join(bids_folder, 'derivatives', 'correlation_matrices.tryNoHalo', f'sub-{sub}_ses-{ses}_task-{task}_confspec-{confspec}_CM-unfiltered.npy')
    np.save(fn, cm)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--bids_folder', default='/mnt_AdaBD_largefiles/Data/SMILE_Data/DNumRisk/ds-stressrisk')
    parser.add_argument('--confspec', default='36P')
    parser.add_argument('--task', default='risk')
