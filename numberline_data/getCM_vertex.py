...
def append_to_csv(subject, N_usable_frames, csv_fn):
    if os.path.exists(csv_fn):
        df = pd.read_csv(csv_fn)
    else:
        df = pd.DataFrame(columns=['subject', 'usabele_frames'])  # Initialize empty DataFrame
    df = pd.concat([df, pd.DataFrame({'subject': [subject], 'usable_frames': [N_usable_frames]})], ignore_index=True)
...
def cleanTS(sub, fmriprep_confounds_include, bids_folder, ses =1, 
    ...
    return clean_ts, usable_frames


def main(subject, bids_folder,  confspec='36P', ses=1, task ='digitorder',space = 'fsaverage5'):
    
    target_folder_cm = op.join(bids_folder, 'derivatives', f'correlation_matrices.{space}')
    os.makedirs(target_folder_cm) if not op.exists(target_folder_cm) else None
    #sub = f'{int(subject):02d}' # 3 digit format for numberline study!

    # define confounds to include
    ...
    clean_ts, N_usable_frames = cleanTS(subject, fmriprep_confounds_include, bids_folder, ses=ses, task = task, space = space)
    csv_fn = op.join(target_folder_cm, 'usable_frames_per_subject.csv')
    append_to_csv(subject, N_usable_frames, csv_fn)

    mask, labeling_noParcel = get_basic_mask() # destrieux atlas mask
    ...
    target_fn = op.join(target_folder_cm,f'sub-{subject}_ses-{ses}_task-{task}_confspec-{confspec}.npy') # _glasserParcel-{space}
